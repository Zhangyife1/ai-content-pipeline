...
一套面向 **AI 增长工程师 / 高级 AI 开发工程师** 面试与实战的端到端内容生产系统：从知识库投喂（RAG）到多步生成、三层质检、多渠道分发，再到数据回流闭环。项目基于真实生产架构抽象而来，核心代码零外部服务即可运行，生产组件通过可选依赖平滑切换。

[![CI](https://github.com/your-name/ai-content-pipeline/actions/workflows/ci.yml/badge.svg)](https://github.com/your-name/ai-content-pipeline/actions/workflows/ci.yml)
...
| API / 任务 | FastAPI 全链路 REST API、Celery Worker、独立发布调度循环 | `src/ai_content_pipeline/api/`、`workers/` |
...
```bash
git clone https://github.com/your-name/ai-content-pipeline.git
...
你会看到：示例知识库增量投喂 → HyDE 混合检索 → 文章生成（大纲/正文/摘要/FAQ/SEO）→ 三层质检 → 定时发布 → 发布成功。
...
```bash
pip install -e ".[dev]"
python -m unittest discover -s tests -v   # 28 个测试：分块/向量/质检/生成/调度/投喂/API
...
│   ├── api/              # FastAPI 路由（投喂/检索/生成/质检/发布）
│   ├── ingestion/        # Connector、清洗、分块、Embedding、向量库、投喂管线
...
│   └── cli.py            # demo / seed / ingest / generate / check / serve
├── tests/                # 28 个单元 + API 测试
...
5. **低耦合扩展**：Connector、Adapter、VectorStore、Scheduler 全部接口化，替换实现不影响上游。
6. **零成本可演示**：demo 模式不依赖任何外部服务，面试可现场跑通全链路。
